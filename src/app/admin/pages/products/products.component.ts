...
import { ProductService, CreateProductDto } from '@shared/services/product.service';
...
export class ProductsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      bootstrap: this._orgService.bootstrap(),
      brands:    this._brandService.getAll(),
    }).pipe(takeUntil(this._destroy$)).subscribe(({ bootstrap, brands }) => {
  ...
  openEdit(product: Product): void {
    ...
    this._productService.getOne(product.id).pipe(takeUntil(this._destroy$)).subscribe((p) => {
      this.presentationsArray.clear();
      ...
      (p.presentations ?? []).forEach((pres) => this._addPresentationRow(pres.unitOfMeasureId, pres.sku ?? ''));
  ...
  save(): void {
    ...
    if (editingId) {
      this._productService.update(editingId, dto)
        .pipe(takeUntil(this._destroy$))
        .subscribe({ next: onSuccess, error: onError });
    } else {
      this._productService.create(dto)
        .pipe(takeUntil(this._destroy$))
        .subscribe({ next: onSuccess, error: onError });
    }
  }
  ...
  doDelete(id: string): void {
    this._productService.remove(id).pipe(takeUntil(this._destroy$)).subscribe({
      next: () => {
        this._deletingId.set(null);
