...
export class CategoriesComponent implements OnInit, OnDestroy {
  ...
  private _load(): void {
    ...
    this._categoryService.getPaginated({
  ...
  doDelete(id: string): void {
    this._categoryService.remove(id).pipe(takeUntil(this._destroy$)).subscribe({
      next: () => {
        this._deletingId.set(null);
        if (this._categories().length === 1 && this._page() > 1) this._page.update((p) => p - 1);
        this._load();
      },
  ...
  onFilesSelected(event: Event): void {
    ...
    this._uploadService.uploadImages('categories', files).pipe(takeUntil(this._destroy$)).subscribe({
      next: (variants) => {
